...
{
    int samples_read, nCur = 0;
    FILE *file = wave_info->soundfile;

	void *pPCM = pcm; 
	unsigned short int* ptrA = NULL;

	samples_read = fread(pPCM, sizeof(char)*2, numSamples, file);

	//nCur = fseek( file, -samples_read, SEEK_CUR );
	//memcpy( pcm, pPCM, numSamples*2 );
    
//	ptrA = reinterpret_cast<unsigned short int*>(pPCM);
	//memcpy( pcm, pPCM, numSamples*sizeof(unsigned int) );
